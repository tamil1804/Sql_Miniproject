-- 1) Create DataBase BANK and create tables
...
-- 2) Insert records
insert into branch_mstr values(1,'Delhi');
insert into branch_mstr values(2,'Mumbai');
...
insert into account_details values(1,1,1,10000,'saving','2012-12-15','Active');
insert into account_details values(2,2,2,5000,'saving','2012-06-12','Active');
...
select fname,dob from customer 
where fname = 'ramesh';

-- 6) Add column city to branch table 
alter table branch_mstr add column city varchar(20) not null;
...
select min(curbal) as min_bal from account_details where custid = any(
select custid from customer);

-- 11) Give the count of customer for each occupation
select occupation,count(custid) as cust_count
from customer
group by occupation;

-- 12) Write a query to find the name (first_name, last_name) of the employees who are managers.
select e1.fname as first_name , e2.lname as last_name 
from employee e1 join employee e2
on e1.emp_number = e2.manager_no;
...
-- 15) Write a query to display the customer number, customer firstname, account number for all the customers
select c.custid as customer_id,fname as first_name ,acnumber as account_no
from customer c join account_details ad
on c.custid = ad.custid;
...
select c.custid as customer_id,fname as first_name , bid as branch_id, curbal as bal_amount
from customer c join account_details ad
on c.custid = ad.custid;

-- 17) Create a virtual table to store the customers who are having the accounts in the same city as they live
create or replace view customers as(
select c.custid, concat(fname,'',lname) as full_name, name as branch_location 
from customer c join account_details ad
on c.custid = ad.custid
join branch_mstr as bm
on bm.branch_no = ad.bid);
select * from customers;

-- 19) Write a query to display the details of customer with second highest balance 
with table1 as(
select c.custid as customer_id, fname as first_name, curbal as balance_amount, dense_rank() over(order by curbal desc) as ranking
from customer c join account_details ad 
on c.custid = ad.custid)
select customer_id, first_name, balance_amount from table1 
where ranking = 2;
...
select * from product;
select product_id, product_desc, product_class_code, case
...
select product_desc , product_class_desc, product_price, oh.order_id , order_status
...
select product_id,product_desc,product_class_desc,pc.product_class_code ,product_quantity_avail, case
when pc.product_class_code in(2050,2053) and product_quantity_avail between 1 and 10 then 'Low stock'
when pc.product_class_code in(2050,2053) and product_quantity_avail between 11 and 30 then 'In stock'
when pc.product_class_code in(2050,2053) and product_quantity_avail > 30 then 'Enough stock'
when pc.product_class_code in(2052,2056) and product_quantity_avail between 1 and 20 then 'Low stock'
when pc.product_class_code in(2052,2056) and product_quantity_avail between 21 and 80 then 'In stock'
when pc.product_class_code in(2052,2056) and product_quantity_avail > 80 then 'Enough stock'
when pc.PRODUCT_CLASS_CODE not in (2052,2056,2050,2053) and product_quantity_avail between 1 and 15 then 'Low stock' 
when pc.PRODUCT_CLASS_CODE not in (2052,2056,2050,2053) and product_quantity_avail between 16 and 50 then 'In stock' 
when pc.PRODUCT_CLASS_CODE not in (2052,2056,2050,2053) and product_quantity_avail > 51 then 'Enough stock'
else 'Out of stock'
end as stock_list
from product p join product_class pc
on p.product_class_code = pc.product_class_code;
select distinct product_class_code from product_class;

-- 4) List customers from outside Karnataka who haven’t bought any toys or books
select state from address;
select distinct customer_id,customer_fname from online_customer where address_id = any(
select address_id from address where state <> 'karnataka') and customer_id = any(
select customer_id from order_header where order_id = any(
select order_id from order_items where product_id = any(
select product_id from product where product_class_code = any(
select product_class_code from product_class where product_class_desc not in ('toys','books')))));